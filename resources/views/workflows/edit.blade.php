...
@section('content')
    <div class="p-6 lg:p-8" x-data="workflowEditor(@js($workflow))">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="mb-8">
                ...
                <h1 class="text-3xl font-semibold">Edit Workflow</h1>
            </div>
            ...
            <form method="POST" action="{{ route('workflows.update', $workflow) }}" class="space-y-8">
                ...
                <!-- Statuses -->
                <div class="bg-white dark:bg-[#161615] rounded-lg shadow-sm p-6 border border-[#e3e3e0] dark:border-[#3E3E3A]">
                    <h2 class="text-xl font-semibold mb-6 text-[#1b1b18] dark:text-[#EDEDEC]">Statuses</h2>
                    ...
                    <div class="space-y-3 mb-4" x-ref="statusContainer">
                        <template x-for="(status, index) in form.statuses" :key="index">
                            <div class="flex items-center gap-3 p-3 bg-[#f5f5f5] dark:bg-[#0a0a0a] rounded-sm border border-[#e3e3e0] dark:border-[#3E3E3A]">
                                ...
                                <!-- Hidden ID field for existing statuses -->
                                <input type="hidden" x-model="form.statuses[index].id" :name="'statuses[' + index + '][id]'">
                                ...
                                <input type="text"
                                       ...
                                       maxlength="40"
                                       required
                                       :readonly="form.statuses[index].name === 'Open' || form.statuses[index].name === 'Closed'"
                                       :class="(form.statuses[index].name === 'Open' || form.statuses[index].name === 'Closed') ? 'cursor-not-allowed opacity-60' : ''"
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <label class="relative inline-flex items-center" :class="(form.statuses[index].name === 'Open' || form.statuses[index].name === 'Closed') ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'">
                                        <input type="checkbox"
                                               x-model="form.statuses[index].is_active"
                                               :name="'statuses[' + index + '][is_active]'"
                                               value="1"
                                               :disabled="form.statuses[index].name === 'Open' || form.statuses[index].name === 'Closed'"
                                               class="sr-only peer">
                                ...
                                <!-- Delete Button - Hidden for Open and Closed -->
                                <div class="w-5">
                                    <button type="button"
                                            x-show="form.statuses[index].name !== 'Open' && form.statuses[index].name !== 'Closed'"
                    ...
                    <p class="mt-4 text-xs text-[#706f6c] dark:text-[#A1A09A]">
        ...
        function workflowEditor(workflow) {
            return {
                form: {
                    ...
                    statuses: Array.isArray(workflow.statuses)
                        ? workflow.statuses
                ...
                removeStatus(index) {
                    if (this.form.statuses.length > 1) {
