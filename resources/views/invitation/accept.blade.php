...
@section('content')
    <div x-data="{ showPassword: false, showConfirmPassword: false }">
        <div class="mb-6">
            <h2 class="text-2xl font-semibold mb-2">Join {{ $company->name }}</h2>
        ...
        <form method="POST" action="{{ route('invitation.accept.submit') }}" class="space-y-4">
            @csrf
            ...
            <div>
                <label for="company_name" class="block text-sm font-medium mb-2">Company</label>
                <input
                    type="text"
                    id="company_name"
                    value="{{ $company->name }}"
            ...
            <div>
                <label for="about_yourself" class="block text-sm font-medium mb-2">About Yourself</label>
                <textarea
                    id="about_yourself"
                    name="about_yourself"
                    rows="3"
                ...
                <p class="mt-1 text-xs text-[#706f6c] dark:text-[#A1A09A]">
                    Optional - share your role, interests, or anything you'd like the team to know
                </p>
            </div>
            ...
            <div>
                ...
                >
                    ...
                    @foreach ($timezones as $value => $label)
                        <option value="{{ $value }}" {{ old('timezone', $existingUser?->timezone) === $value ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
